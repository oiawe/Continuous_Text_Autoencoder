import os
import torch
import torch.nn as nn
import torch.optim as optim
from torch.nn.parallel import DistributedDataParallel as DDP

def continue_training(checkpoint_path, model: DDP, optimizer: optim.Optimizer) -> int:
    """load the latest checkpoints and optimizers"""
    model_dict = {}
    optimizer_dict = {}
    
    # globt all the checkpoints in the directory
    for file in os.listdir(checkpoint_path):
        if file.endswith(".pt") and '_' in file:
            name, step_str = file.rsplit('_', 1)
            step = int(step_str.split('.')[0])
            
            if name.startswith("checkpoint"):
                model_dict[step] = file
            elif name.startswith("optimizer"):
                optimizer_dict[step] = file
    
    # get the largest step
    common_steps = set(model_dict.keys()) & set(optimizer_dict.keys())
    if common_steps:
        max_step = max(common_steps)
        model_path = os.path.join(checkpoint_path, model_dict[max_step])
        optimizer_path = os.path.join(checkpoint_path, optimizer_dict[max_step])
        
        # load model and optimizer
        model.module.load_state_dict(torch.load(model_path, map_location='cpu'))
        optimizer.load_state_dict(torch.load(optimizer_path, map_location='cpu'))
        
        print(f'resume model and optimizer from {max_step} step')
        return max_step + 1
    
    else:
        # load pretrained checkpoint
        if model_dict:
            model_path = os.path.join(checkpoint_path, model_dict[max(model_dict.keys())])
            model.module.load_state_dict(torch.load(model_path, map_location='cpu'))
            
        return 0

def get_model_statedict(model: DDP):
    """Remove unwanted key in torch.compile"""
    state_dict = model.module.state_dict()
    unwanted_prefix = '_orig_mod.' 
    for k,v in list(state_dict.items()):
        if unwanted_prefix in k: 
            state_dict[k.replace(unwanted_prefix, '')] = state_dict.pop(k) 
    return state_dict